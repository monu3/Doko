      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          onClick={() => handleHomeClick(onHomeSelect)}>
            <img 
              src={shop?.logoUrl ||"https://images.unsplash.com/photo-1635151947209-b60373e4ccc5"} 
              alt="shop Logo" 
              className="w-10 h-10 rounded-full object-cover"
            />
            <span className="text-xl font-bold text-green-800">{shop?.businessName}</span>
          ...
          <nav className="hidden md:flex space-x-8">
            ...
            <div
            className={`cursor-pointer text-green-700 hover:text-green-600 font-medium transition-colors
              ${!selectedCategory ? 'text-green-800 underline' : 'text-gray-600'}`}
            onClick={() => handleHomeClick(onHomeSelect)}
            >
              Home
            </div>
            ...
            {visibleCategories.map((category) => (
              <div
                key={category.id}
                className={`hover:text-green-600 font-medium transition-colors cursor-pointer ${
                  selectedCategory === category.id
                    ? "text-green-800 underline"
                    : "text-gray-600"
                }`}
                onClick={() => handleCategoryClick(category.id, category.name, onCategorySelect)}
              >
                {category.name}
              </div>
            ))}
            ...
            {dropdownCategories.length > 0 && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className={`hover:text-green-600 font-medium transition-colors p-0 h-auto ${
                ...
                <DropdownMenuContent align="center" className="w-48">
                  {dropdownCategories.map((category) => (
                    <DropdownMenuItem
                      ...
                      onClick={() =>
                        handleCategoryClick(category.id, category.name, onCategorySelect)
                      }
                    >
                      {category.name}
                    </DropdownMenuItem>
            ...
            <div className="flex flex-col space-y-3">
              ...
              <div
                className={`text-green-700 font-medium cursor-pointer ${
                  !selectedCategory ? "text-green-800 underline" : ""
                }`}
                onClick={() => {
